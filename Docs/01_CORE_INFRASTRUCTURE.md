...
The app cleanly separates the _manager_ from the _reactive state_.
...
5. **`ErrorInterceptor`**: Catches all Dio exceptions and maps them into custom domain `AppException` types. It extracts RFC 7807 `ProblemDetails`/`ValidationProblemDetails` messages, joins validation errors, and prefers `title + detail` when both are present.
...
_End of Phase 2.1 Analysis. Next: Phase 2.2 - Design System & Common UI Layer._
